...
export const InputText = props => {
  ...
  const getRightIcon = () => {
    if(!rightIcon && secureTextEntry){
      return <TouchableOpacity style={{paddingHorizontal:10}} onPress={()=>setSecure(!isSecure)}>
        <Icon name={isSecure ?'eye-slash' :'eye'} size={15} color={COLORS.darkGray} />
      </TouchableOpacity>
    }else if(isPressable){
  ...
  return (
    <View
      ...
      <View style={{
        ...
        {isPressable ? <TouchableOpacity onPress={onPress} style={{flex:1}}>
          <Typography style={{
            fontSize: FONTSIZE.S,
          ...
          }}>{startCase(value || placeholder)}</Typography>
        </TouchableOpacity> :
        <TextInput
          style={{
            flex: 1,
